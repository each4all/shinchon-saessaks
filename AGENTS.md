...
## History Log
- 2025-11-20 — 공통 SectionSidebar 도입: `src/components/navigation/SectionSidebar.tsx`로 사이드바 카드/링크 스타일과 활성화 로직을 통합하고, 유치원 소개·교육환경·교육과정·우리들 이야기 페이지가 모두 해당 컴포넌트를 사용하도록 리팩터링(인사말 탭별 활성 상태 유지).
- 2025-11-20 — 갤러리 라이트박스 전환: 카드 그리드 + 공통 라이트박스로 재구성하고, 썸네일 휠/드래그 가로 스크롤·활성 썸네일 중앙 정렬·Home/End/←/→ 지원으로 다량 이미지 탐색을 단순화했다. 페이드/드래그 오동작 제거, 썸네일·제목·설명을 이미지 하단에 분리해 가려짐 문제를 해결.
  - 2025-11-20 — 라이트박스 내비 보완: 메인 이미지 클릭 전환 제거(화살표/썸네일/키보드 중심), 썸네일 중앙 정렬·메타 표시 유지, 썸네일 클릭 전환 안정화.
- 2025-11-20 — 반별 교육활동 페이지에서 연령별 앨범 블록을 제거(연령별 앨범은 별도 페이지에서 유지).
...
- 2025-11-20 — 사이드바 공통화/플로팅 준비: `SectionSidebar`를 도입해 유치원 소개·교육환경·교육과정·우리들 이야기 사이드바 UI를 통일하고, desktop 기준 `stickyOffset=112px` 상단 플로팅을 적용(인사말 탭 포함).
- 2025-11-21 — 푸터 슬림화: 3열(1:1:1) 가로 레이아웃·축소 패딩으로 높이를 줄여 sticky 사이드바 간섭을 완화하고, 빠른 연결/연락처를 좌측 정렬로 단순화. (확인 중)
